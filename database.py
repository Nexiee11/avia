...
class DB:
    ...
    def check_user_in_db(self,input_user_id):
        res = self.cursor.execute("SELECT user_id FROM users WHERE user_id = ?",(input_user_id,))
        return bool(len(res.fetchall()))
    def get_user_id(self,input_user_id): 
        res = self.cursor.execute("SELECT id FROM users WHERE user_id = ?",(input_user_id,))
        return res.fetchone()[0]
    
    def add_user(self,input_user_id,home_port='MOW'):
        self.cursor.execute("INSERT INTO users ('user_id','home_airport','is_admin') VALUES(?,?,?)",(input_user_id,home_port,0))
    ...
    def count_user_searches(self,user_id):
        res = self.cursor.execute("SELECT COUNT(*) FROM flights_search WHERE user_id = ?",(user_id))
        return res.fetchall()
    ...
    def check_admin(self):
        res = self.cursor.execute("SELECT user_id FROM users WHERE is_admin = 1",)
        return res.fetchall()[0][0]
